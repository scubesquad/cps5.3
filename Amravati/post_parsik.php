	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		if(!$entries_found)
		{
			foreach ($data as $key => $explode) {
				...
				$addresses1 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', trim($explode[14]))); 
				$addresses2 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', trim($explode[15]))); 
				$addresses3 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', trim($explode[16])));
				$addresses4 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', trim($explode[17])));
				$addresses5 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', trim($explode[18])));

				$name = str_replace("'", "\'", trim($explode[8]));
				...
				$explode[9]=trim($explode[9]);
				$explode[10]=trim($explode[10]);
				$explode[11]=trim($explode[11]);
				$explode[12]=trim($explode[12]);
				$explode[13]=trim($explode[13]);
...
function FetchUploadedFiles(){
	...
	if(isset($_REQUEST['bookSize']) && $_REQUEST['bookSize'] != '')
		$cps_book_size = $_REQUEST['bookSize'];
	...
	if($cps_book_size)
	{
		$queryStr .= " and  cps_book_size = '$cps_book_size'";
	}
