...
$SearchString = "";
	...
	foreach ($result as $key => $row) {
		...
		$account = trim($row->cps_account_no);
		...
		$data .= $brprcode= str_pad($branch_code, 6, "0", STR_PAD_LEFT). $product_code ;

		$strlenbrprcode=strlen($brprcode);
		$totalStrLen=$strlenbrprcode+8;
		$paddingSpace=30-$totalStrLen;
		$data .= str_pad($acc6, $paddingSpace, " ", STR_PAD_LEFT) . $pdate;//13
