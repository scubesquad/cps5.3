...
//print_r($_REQUEST);
	...
	if(isset($_GET['cps_books_size']) && !empty($_GET['cps_books_size'])){
		$condation .= ' AND prc.cps_book_size = "'.$_GET['cps_books_size'].'"';
	}
	$condation .= ' AND prc.chq_printed_date = "'.date('Y-m-d').'" AND prc.cps_is_reprint=0';
...
{
	$sql1 = "select branch_name from  tb_branchdetails where branch_code = ".$_GET['branchcode'];
...
if($result = $db->get_results($sql)){
	...
	$data .= '<table cellpadding="3" cellspacing="0" border="1" width="100%">
			...
			<td width="10%" class="thwidthth" align="center">Unique Req. No.</td>								
			<td width="13%" height="35px" class="thwidthth" align="center">Account. No</td>
			<td width="7%" class="thwidthth" align="center">Branch ID</td>								
			<td width="12%" class="thwidthth" align="center">Customer Name</td>
			<td width="5%" class="thwidthth" align="center">A/C. Type</td>								
			...
			<td width="8%" class="thwidthth" align="center">User</td>
	...
	foreach($result as $row) 
	{
		$data .= '<tr>
			...
			<td class="thwidthtd" align="center">'.$row->cps_process_user_id.'</td>	
