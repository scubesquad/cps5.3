<?php require_once('global.php');
	...
	else if(isset($_POST['txtlastno']) && !empty($_POST['txtlastno']) && $_POST['txtlastno'] != "") 
	{
		if(strlen($_POST['txtlastno']) != 6){
			...
			{
				$row_series_branch_code = $db->get_row("SELECT branch_code, branch_sub_code FROM tb_branchdetails WHERE branch_id = '".GetSQLValueString($_POST['ddlBranch'],'int')."' ");
				$sql = "INSERT INTO tb_cps_chequeseries (series_transationcode,series_branchcode,series_from,series_to,series_lastno,serise_Bank,serise_branchcode_branch, branch_sub_code) VALUES ('".GetSQLValueString($_POST['ddlTransactioncode'],'int')."','".GetSQLValueString($_POST['ddlBranch'],'int')."','".GetSQLValueString($_POST['txtFROM'],'int')."','".GetSQLValueString($_POST['txtTo'],'int')."','".GetSQLValueString($_POST['txtlastno'],'int')."','1','".$row_series_branch_code->branch_code."','".$row_series_branch_code->branch_sub_code."')";
				$db->query($sql);	
				echo '{"status":"true"}';
				exit();
			}			
