...
function ASCIIFileOutput($fromdate, $todate, $branch,$cps_tr_code)
{
	...
	{
		if($count == 1){
			$SearchString .= " where chq_printed_date between '".date('Y-m-d',strtotime($fromdate))."' and '".date('Y-m-d', strtotime($todate))."' ";
		}else{
			$SearchString .= " and chq_printed_date between '".date('Y-m-d',strtotime($fromdate))."' and '".date('Y-m-d', strtotime($todate))."' ";			
		}
		$count++;
	}
	if($branch != "")
	...
	{
		if($count == 1){
			$SearchString .= " where cps_tr_code = '".$cps_tr_code."' ";
		}else{
			$SearchString .= " and cps_tr_code = '".$cps_tr_code."' ";
		}
		$count++;
	}
...
if((isset($_REQUEST['frm']) && isset($_REQUEST['to'])) || (isset($_REQUEST['ddlBranchName']))){
	...
	if($_REQUEST['cps_tr_code'] != ""){
		$cps_tr_code = $_REQUEST['cps_tr_code'];
	}
	else
	{
		$cps_tr_code = "";
	...
	if($result->outputfileformat == "ASCII"){
		ASCIIFileOutput($fromdate, $todate, $branch, $cps_tr_code);
	}
	else if($result->outputfileformat == "Excel"){
		ExcelOutput($fromdate, $todate, $branch, $cps_tr_code);
	}
}
