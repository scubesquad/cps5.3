...
$pdf->SetHeaderData('', '', 'Cheque Book Print Report '.$filter, '');
...
if(isset($_REQUEST['ddlBranchName'])&&!empty($_REQUEST['ddlBranchName']))
{
    ...
    //$sql = "(select id,cps_unique_req, cps_branchmicr_code,cps_account_no,cps_act_name,cps_act_jointname1,cps_no_of_books,cps_book_size,cps_chq_no_from,cps_chq_no_to,chq_printed_date,cps_tr_code,cps_process_user_id from tb_print_req_collection where chq_printed_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." ORDER BY cps_branchmicr_code ASC,cps_tr_code ASC,chq_printed_date ASC)";

     $sql = "select id,cps_unique_req, cps_branchmicr_code,cps_account_no,cps_act_name,cps_act_jointname1,cps_no_of_books,cps_book_size,cps_chq_no_from,cps_chq_no_to,chq_printed_date,cps_tr_code,cps_process_user_id from tb_print_req_collection where chq_printed_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." ORDER BY cps_unique_req asc";//,ABS(cps_branchmicr_code) ASC,ABS(cps_tr_code) ASC,chq_printed_date ASC, id ASC
    ...
    if($result = $db->get_results($sql)){
                ...
                $data .='<tr style="font-weight:bold; border:1px solid black;">
                        <td width="10%" class="thwidthth" align="center">Unique Request No.</td>
                        <td width="13%" class="thwidthth" align="center">Acc. No</td>
                        <td width="7%" class="thwidthth" align="center">Branch Code</td>
                        <td width="12%" class="thwidthth" align="center">Customer Name</td>  
                        <td width="5%" class="thwidthth" align="center">Acc. Type</td>
                        <td width="7%" class="thwidthth" align="center">Chq. No. From</td>
                        <td width="7%" class="thwidthth" align="center">Chq. No. To</td>                            
                        <td width="5%" class="thwidthth" align="center">No Of Books</td>
                        <td width="5%" class="thwidthth" align="center">Book Size</td>
                        <td width="8%" class="thwidthth" align="center">Print Date</td>
                        <td width="8%" class="thwidthth" align="center">User Name</td>
                        <td width="5%" class="thwidthth" align="center">Br. Id.</td>
            ...
            foreach($result as $row) {
                  ...
                  /*switch ($prevTrCode) {
                            case '10':
                                      $data .= '<tr style="font-size:30px;">
                                        <td class="thwidthtd" colspan="7" align="right"><b>Savings A/c. No.</b></td>
                                        <td class="thwidthtd" align="center"><b>'.$savingCountTotalBook.'</b></td>
                                        <td class="thwidthtd" align="center"><b>'.$savingCountTotalLeaves.'</b></td>
                                        <td class="thwidthtd" colspan="4"></td>
                                        </tr>' ;
                                      break;
                            case '11':
                                       $data .= '<tr style="font-size:30px;">
                                        <td class="thwidthtd" colspan="7" align="right"><b>Current A/c. No.</b></td>
                                        <td class="thwidthtd" align="center"><b>'.$currentCountTotalBook.'</b></td>
                                        <td class="thwidthtd" align="center"><b>'.$currentCountTotalLeaves.'</b></td>
                                        <td class="thwidthtd" colspan="4"></td>
                                        </tr>' ;
                            ...
                            case '17':
                                        $data .= '<tr style="font-size:30px;">
                                        <td class="thwidthtd" colspan="7" align="right"><b>Pay Order No.</b></td>
                            ...
                            case '13':
                                        $data .= '<tr style="font-size:30px;">
                                        <td class="thwidthtd" colspan="7" align="right"><b>Demand Draft No.</b></td>
                            ...
                            default:
                                      $data .= '<tr style="font-size:30px;">
                                        <td class="thwidthtd" colspan="7" align="right"><b></b></td>
                        ...
                        $data .= '<tr>
                                        ...
                                        <td class="thwidthtd" align="center">'.date('d-m-Y', strtotime($row->chq_printed_date)).'</td>
             ...
             /*if($prevTrCode==10){
              ...
              /*switch ($prevTrCode) {
                            ...
                            case '13':
                                        $data .= '<tr style="font-size:30px;">
                                        <td class="thwidthtd" colspan="7" align="right"><b>Cash Credit No.</b></td>
                                        <td class="thwidthtd" align="center"><b>'.$ddCountTotalBook.'</b></td>
                                        <td class="thwidthtd" align="center"><b>'.$ddCountTotalLeaves.'</b></td>
                                        <td class="thwidthtd" colspan="4"></td>
                                        </tr>' ;
               ...
               $data .= '<tr style="font-size:30px;">
                          <td class="thwidthtd" colspan="7" align="right"><b>'.$branch_name.' Total</b></td>
   ...
   // $sqlbranch = "(select distinct cps_branchmicr_code from tb_print_req_collection where chq_printed_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." ORDER BY cps_branchmicr_code ASC)";

     $sqlbranch = "select distinct ABS(cps_branchmicr_code) as cps_branchmicr_code from tb_print_req_collection where chq_printed_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." ORDER BY ABS(cps_branchmicr_code) ASC";
...
if($resultbranch = $db->get_results($sqlbranch)){
  ...
  foreach ($resultbranch as $rowbranch) {
           ...
           // $sql = "(select id,cps_unique_req, cps_branchmicr_code,cps_account_no,cps_act_name,cps_act_jointname1,cps_no_of_books,cps_book_size,cps_chq_no_from,cps_chq_no_to,chq_printed_date,cps_tr_code,cps_process_user_id from tb_print_req_collection where chq_printed_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchStringIndv." ORDER BY cps_tr_code ASC,chq_printed_date ASC)";

            $sql = "select id,cps_unique_req, cps_branchmicr_code,cps_account_no,cps_act_name,cps_act_jointname1,cps_no_of_books,cps_book_size,cps_chq_no_from,cps_chq_no_to,chq_printed_date,cps_tr_code,cps_process_user_id from tb_print_req_collection where chq_printed_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchStringIndv." ORDER BY cps_unique_req asc";//ABS(cps_branchmicr_code) ASC,ABS(cps_tr_code) ASC,chq_printed_date ASC, id ASC
...
print_r($sql);
             ...
             /*switch ($prevTrCode) {
                            ...
                            default:
                                      $data .= '<tr style="font-size:30px;">
                                        ...
                                        <td class="thwidthtd" align="center" colspan="3"><b></b></td>
                                        </tr>' ;
                                      break;
                          }*/
              /* $data .= '<tr style="font-size:30px;">
        	...
        	$data .= '<table cellpadding="2" cellspacing="0" border="1" width="100%">';
              $data .= '<tr style="font-size:30px;">
                          <td class="thwidthtd" colspan="9" align="right"><b>Bank Total</b></td>
              ...
              $data = '<table cellpadding="2" cellspacing="0" border="1" width="100%">';
                ...
                $data .= '<tr><td class="thwidthtd" colspan="13" style="font-size:35px;">No data found!</td></tr>' ;
                $data .= '</table>';
    }
  }
