	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
		   ...
		   /* print_r($explode);
		    ...
		    if(count($explode)==40){
		    	...
		    	$result=searcharray($explode[26], $explode[27], $printReqArr,$explode[22]);
		    	...
		    	if($result){
		    		...
		    		$printReqArr[]=array("cps_chq_no_from"=>$explode[26],"cps_chq_no_to"=>$explode[27],"cps_account_no"=>$explode[22]);
		    	}
				...
				if($result = $db->get_results("select cps_unique_req from tb_print_req_collection where ".$whereStr." (cps_account_no = '$explode[22]' and ((cps_chq_no_from <= '$explode[26]' AND cps_chq_no_to >= '$explode[26]') OR (cps_chq_no_from <= '$explode[27]' AND cps_chq_no_to >= '$explode[27]')))"))
				{
					if($explode[23]==17 || $explode[23]=="17"){
				...
				else if($result = $db->get_results("select cps_unique_req from tb_uploadingdata where ".$whereStr." (cps_account_no = '$explode[22]' and ((cps_chq_no_from <= '$explode[26]' AND cps_chq_no_to >= '$explode[26]') OR (cps_chq_no_from <= '$explode[27]' AND cps_chq_no_to >= '$explode[27]')))"))
				{
					if($explode[23]==17 || $explode[23]=="17"){
					echo "<p class='error-p'>Error: This file has records which are already printed. | A/c No : ".$explode[22]." | Series : ".$explode[26]." to ".$explode[27]."</p>";
					}else{
		...
		if(!$entries_found)
		{
			foreach ($data as $key => $explode) {
				...
				cps_act_mobile, cps_act_telephone_res, cps_act_telephone_off,cps_atpar
				...
				'$explode[28]', '$explode[26]','$explode[27]',
...
function searcharray($value1, $value2, $array,$cps_account_no) {
   foreach ($array as $k => $val) {
   
       if ((($val['cps_chq_no_from'] <= $value1&&$val['cps_chq_no_to'] >= $value1) || ($val['cps_chq_no_from'] <= $value2&&$val['cps_chq_no_to'] >= $value2))&&$val['cps_account_no']==$cps_account_no) {
       	print_r($val);
       	exit;
           return true;
       }
