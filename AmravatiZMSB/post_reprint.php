...
if(isset($_POST["from_date"]) && $_POST["from_date"]!="" && isset($_POST["to_date"]) && $_POST["to_date"]!="")
{
	...
	FetchReprintRecords($fromdate,$todate,$_POST['ddlBranchName'],$_POST['ddlTranType'],$_POST['bookSize']);
}
...
function FetchReprintRecords(){
	...
	if(isset($_REQUEST['ddlBranchName'])&&!empty($_REQUEST['ddlBranchName'])){
		$whereStr .=" AND cps_branchmicr_code = '".$_REQUEST['ddlBranchName']."' ";

	}

	if(isset($_REQUEST['ddlTranType'])&&!empty($_REQUEST['ddlTranType'])){
		$whereStr .=" AND cps_tr_code = '".$_REQUEST['ddlTranType']."' ";

	}

	if(isset($_REQUEST['bookSize'])&&!empty($_REQUEST['bookSize'])){
		$whereStr .=" AND cps_book_size = '".$_REQUEST['bookSize']."' ";

	}


	$result = $db->get_results("SELECT * FROM tb_print_req_collection WHERE cps_date BETWEEN '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$whereStr) ;
