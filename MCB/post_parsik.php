	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
		    ...
		    if(count($explode)==20){
					...
					$multiArr[$count]['acc_no'] = $explode[5];
					$multiArr[$count]['cheque_no'] = $explode[18];
					$count++;
					
					$unique = array_unique($multiArr, SORT_REGULAR);
					$diffCellUniq = array_diff_key($multiArr, $unique);
				
					if($diffCellUniq){
						echo "<p style='color:red'>This file has duplicate serial numbers. Account No - ".$explode[5].". Cheque from series - ".$explode[18]."</p>";
						$entries_found = true;
						break;
					}
					//{

					// array_push($serialNoArr, $explode[18]);
					

					//	echo "select cps_unique_req from tb_print_req_collection where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[18])." and cps_chq_no_to >= ".trim($explode[18]);
					if($result = $db->get_results("select cps_unique_req from tb_print_req_collection where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[18])." and cps_chq_no_to >= ".trim($explode[18])."")) 
					{
						echo "<p style='color:red'>This file has records which are already printed. Account No - ".$explode[5].". Cheque from series - ".$explode[18]."</p>";
						$entries_found = true;
						break;
					}
					else if($result = $db->get_results("select cps_unique_req from tb_uploadingdata where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[18])." and cps_chq_no_to >= ".trim($explode[18]).""))
					{
						echo "<p style='color:red'>This file has records which are already uploaded for printing. Account No - ".$explode[5].". Cheque from series - ".$explode[18]."</p>";
						$entries_found = true;
						break;
					}else if($result = $db->get_results("select cps_unique_req from tb_printque where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[18])." and cps_chq_no_to >= ".trim($explode[18]).""))
					{
						echo "<p style='color:red'>This file has records which are already uploaded for printing. Account No - ".$explode[5].". Cheque from series - ".$explode[18]."</p>";
						$entries_found = true;
						break;
					}else if($result = $db->get_results("select cps_unique_req from tb_pending_print_req where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[18])." and cps_chq_no_to >= ".trim($explode[18]).""))
					{
						echo "<p style='color:red'>This file has records which are already uploaded for printing. Account No - ".$explode[5].". Cheque from series - ".$explode[18]."</p>";
						$entries_found = true;
						break;
				...
				echo "<p style='color:red'>Please upload valid file.</p>";
				$entries_found = true;
				break;
			}
		}
...
/*
				...
				*/
				...
				$cps_branchmicr_code= substr($explode[3], -3);
				$cps_tr_code= trim($explode[6]);
				$cps_account_no= trim($explode[5]);
				$cps_act_name=trim($explode[7]);
				$cps_act_jointname1=trim($explode[8]); 
				$cps_act_jointname2=trim($explode[9]);
				$cps_act_address1=trim($addresses1);
				$cps_act_address2=trim($addresses2);
				$cps_act_address3=trim($addresses3);
				$cps_act_address4="";
				$cps_act_address5="";
