<?php require_once('global.php');
	...
	if(isset($_REQUEST['userid']) && !empty($_REQUEST['userid']) && isset($_REQUEST['password']) && !empty($_REQUEST['password']) ) {	
		...
		$sql="SELECT * , DATE_ADD(create_date, INTERVAL ".$rowsetting->password_expiry." DAY) as total_date, CURDATE() as 'current_date' FROM tb_printadmin WHERE userid like binary '".mysql_real_escape_string($un)."' and password like binary '".mysql_real_escape_string($pass)."' and password_status = 1 ";
		...
		if($count > 0) {
			if(strtotime($row_user->total_date) >= strtotime($row_user->current_date)){
				...
				$sql = "UPDATE tb_printadmin SET incorrect_attempt = 0 WHERE userid = '".$_REQUEST['userid']."'";
				$db->query($sql);
				echo '{"status":"true"}';
				exit();
			}
				...
				unset($sumbit);
				echo '{"error":"true", "htmlcontent":"Your password has expired please contact to administrator."}';
				exit(0);
			}
		}
		else {
