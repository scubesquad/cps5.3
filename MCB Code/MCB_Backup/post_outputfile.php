...
function PipeFileOutput($fromdate, $todate, $branch)
{
	header("Content-Type: plain/text");
	header("Content-Disposition: Attachment; filename=Output-".$fromdate."to".$todate.".txt");
	header("Pragma: no-cache");
	...
	if($result = $db->get_results($sql)) {
		...
		foreach($result as $row) {									
			...
			$noofbooks = stripslashes($row->cps_no_of_books);
			$booksize = stripslashes($row->cps_book_size);
			//$bearOrder = stripslashes($row->cps_act_name);
			//$atpar = stripslashes($row->cps_atpar);
			if($atpar == "1"){
				$atpar = "Y";
				$isBearer = "";
			...
			//010722111435|DHRUVA DURGESH KUNCHIKORVE|DURGESH RAMESH KUNCHIKURVE|||||BLOCK NO.8,ROW-1,R.NO.7,RAJIV GANDHI NAG|AR,NR.SHANKAR STORES,TRANSIST CAMP,|DHARAVI|||Mumbai Bra|400017||||400|110|018||305364|301000015305364|10|01|015|254961|254975|Bearer|N|||||||||||

			$chunk = 3;
			$arr = str_split($row->cps_micr_code, $chunk);
			$cityCode=$arr[0];
			$bankCode=$arr[1];
			$branchCode=$arr[2];
			$subAcc=substr($accountno, -6);

			$outString .=  $uniqueReq."|".stripslashes($row->cps_act_name)."|".stripslashes($row->cps_act_jointname1)."|".stripslashes($row->cps_act_jointname2)."||||".stripslashes($row->cps_act_address1)."|".stripslashes($row->cps_act_address2)."|".stripslashes($row->cps_act_address3)."|||".stripslashes($row->cps_act_city)."|".stripslashes($row->cps_act_pin)."||||".stripslashes($cityCode)."|".stripslashes($bankCode)."|".stripslashes($branchCode)."||".stripslashes($subAcc)."|".stripslashes($accountno)."|".$branchcode."|".$noofbooks."|".$booksize."|".$chknofrom."|".$chknoto."|".$isBearer."|".$atpar."|||||||||||"."\r\n";
		}
	}
	echo $outString;
}
...
if((isset($_REQUEST['frm']) && isset($_REQUEST['to'])) || (isset($_REQUEST['ddlBranchName']))){
	...
	}else if($result->outputfileformat == "Pipe"){
		PipeFileOutput($fromdate, $todate, $branch);
	}
