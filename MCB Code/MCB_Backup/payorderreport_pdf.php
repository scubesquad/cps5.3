...
$condation = ' 1=1 ';
	if(isset($_REQUEST['branchid']) && !empty($_REQUEST['branchid'])){
		$condation .= ' AND tb_payorder_req_collection.cps_branchmicr_code = "'.$_REQUEST['branchid'].'"';
	}
	if(isset($_GET['cps_book_size']) && !empty($_GET['cps_book_size'])){
		$condation .= ' AND tb_payorder_req_collection.cps_book_size = "'.$_GET['cps_book_size'].'"';
	}
	if(isset($_GET['frm']) && !empty($_GET['frm']) && isset($_GET['to']) && !empty($_GET['to'])){
		$condation .= ' AND (tb_payorder_req_collection.cps_date between "'.date('Y-m-d',strtotime($_REQUEST['frm'])).'" and "'.date('Y-m-d', strtotime($_REQUEST['to'])).'")';
	}
	
	$sql = "select tb_payorder_req_collection.*,b.branch_name, p.userid from tb_payorder_req_collection inner join tb_printadmin p on tb_payorder_req_collection.cps_process_user_id = p.adminid LEFT OUTER JOIN tb_branchdetails b ON b.branch_code = tb_payorder_req_collection.cps_branchmicr_code where ".$condation."";
