...
error_reporting(E_ALL ^ E_NOTICE);
	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
			...
			if($result = $db->get_results("select cps_unique_req from tb_print_req_collection where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from = ".trim($explode[18]).""))
			{
			...
			else if($result = $db->get_results("select cps_unique_req from tb_uploadingdata where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from = ".trim($explode[18]).""))
			{
				...
				$entries_found = true;
				break;
			}else if($result = $db->get_results("select cps_unique_req from tb_printque where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from = ".trim($explode[18]).""))
			{
				echo "<p style='color:red'>This file has records which are already uploaded for printing. Account No - ".$explode[5].". Cheque from series - ".$explode[18]."</p>";
				$entries_found = true;
				break;
			}else if($result = $db->get_results("select cps_unique_req from tb_pending_print_req where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from = ".trim($explode[18]).""))
			{
				echo "<p style='color:red'>This file has records which are already uploaded for printing. Account No - ".$explode[5].". Cheque from series - ".$explode[18]."</p>";
				$entries_found = true;
				break;
