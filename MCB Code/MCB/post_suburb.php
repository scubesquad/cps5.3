...
if(isset($_REQUEST['do']) && ($_REQUEST['do'] == 'delsub') && isset($_REQUEST['suid']) && !empty($_REQUEST['suid']) ) {
		...
		if(isset($_REQUEST['txtsuburb']) && !empty($_REQUEST['txtsuburb'])) {
			if($result = $db->get_results("Select * from tb_suburbmaster where suburb_name = '".$_REQUEST['txtsuburb']."' AND suburb_postal_code = '".$_REQUEST['txtpostalcode']."' and is_delete = 0")){
		...
		if(isset($_REQUEST['txtsuburb']) && !empty($_REQUEST['txtsuburb'])) {		
			if($result = $db->get_results("Select * from tb_suburbmaster where suburb_name = '".$_REQUEST['txtsuburb']."' AND suburb_postal_code = '".$_REQUEST['txtpostalcode']."' and suburb_id not in (".$_REQUEST['suid'].") and is_delete = 0")){
				echo '{"error":"true", "htmlcontent":"duplicat suburb"}';
				$db->closeDb();
				exit();
			}
