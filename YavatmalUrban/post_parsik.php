	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
		    ...
		    if(count($explode)==40){
		    	...
		    	$branch_sub_code = substr($explode[22], 0, 2);

		    	$result=searcharray($explode[26], $explode[27],$explode[23], $branch_sub_code,$printReqArr);
		    	...
		    	if($result){
		    		...
		    		$printReqArr[]=array("cps_chq_no_from"=>$explode[26],"cps_chq_no_to"=>$explode[27],"cps_tr_code"=>$explode[23],"branch_sub_code"=>$branch_sub_code);
		    	}

		    	if($explode[23]==17 || $explode[23]=="17"){
				...
				if($result = $db->get_results("select cps_unique_req from tb_print_req_collection where ".$whereStr." ( cps_tr_code=".$explode[23]." AND cps_branchmicr_code=".$cpsbranch_code." AND branch_sub_code=".$branch_sub_code." AND ((cps_chq_no_from <= '$explode[26]' AND cps_chq_no_to >= '$explode[26]') OR (cps_chq_no_from <= '$explode[27]' AND cps_chq_no_to >= '$explode[27]')))"))
				{
					if($explode[23]==17 || $explode[23]=="17"){
				...
				else if($result = $db->get_results("select cps_unique_req from tb_uploadingdata where ".$whereStr." ( cps_tr_code=".$explode[23]." AND cps_branchmicr_code=".$cpsbranch_code." AND branch_sub_code=".$branch_sub_code." AND ((cps_chq_no_from <= '$explode[26]' AND cps_chq_no_to >= '$explode[26]') OR (cps_chq_no_from <= '$explode[27]' AND cps_chq_no_to >= '$explode[27]')))"))
				{
					if($explode[23]==17 || $explode[23]=="17"){
					echo "<p class='error-p'>Error: This file has records which are already printed. | A/c No : ".$explode[22]." | Series : ".$explode[26]." to ".$explode[27]."</p>";
					}else{
		...
		if(!$entries_found)
		{
			foreach ($data as $key => $explode) {
				...
				*/
				...
				'$addresses4','$addresses5','$explode[12]','$explode[13]','$explode[24]', '$explode[25]',
				...
				//	$cps_micr_code = substr($explode[17], -3).substr($explode[18], -3).substr($cpsbranch_code, -3);
					...
					cps_act_mobile, cps_act_telephone_res, cps_act_telephone_off,cps_bsr_code,bank_name, branch_sub_code
...
function searcharray($value1, $value2, $trCode, $subCode, $array) {
   foreach ($array as $k => $val) {
   
       if ((($val['cps_chq_no_from'] <= $value1&&$val['cps_chq_no_to'] >= $value1) || ($val['cps_chq_no_from'] <= $value2&&$val['cps_chq_no_to'] >= $value2))&&$val['cps_tr_code']==$trCode&&$val['branch_sub_code']==$subCode) {
       	print_r($val);
       	exit;
           return true;
       }
