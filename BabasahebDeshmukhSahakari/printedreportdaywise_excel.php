...
require_once(ROOT_CLASSES.'PHPExcel.php');
	...
	$condation = ' 1=1 ';
	if(isset($_REQUEST['ddlBranchName']) && !empty($_REQUEST['cps_branchmicr_code'])){
		$condation .= ' AND prc.cps_branchmicr_code = "'.$_REQUEST['cps_branchmicr_code'].'"';
	}
	if(isset($_GET['ddlTranType']) && !empty($_GET['ddlTranType'])){
		$condation .= ' AND prc.cps_tr_code = "'.$_GET['ddlTranType'].'"';
	}
