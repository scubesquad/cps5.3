...
<?php
	...
	{*/	
		...
		if($rowresults = $db->get_results("SELECT * FROM tb_pending_print_req where cps_isprint = 1"))
		{
			...
			{
				//echo "SELECT series_lastno FROM tb_cps_chequeseries WHERE series_transationcode = $results->cps_tr_code AND serise_branchcode_branch = $results->cps_branchmicr_code";
