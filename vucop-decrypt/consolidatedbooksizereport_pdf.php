...
if(isset($_REQUEST['ddlBranchName'])&&!empty($_REQUEST['ddlBranchName']))
{
  ...
  //  $sql = "(select cps_micr_code,sum(cps_no_of_books) as cps_no_of_books,cps_tr_code,cps_book_size from tb_print_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." GROUP BY cps_micr_code,cps_tr_code,cps_book_size ORDER BY cps_micr_code ASC,cps_tr_code,cps_tr_code ASC,cps_book_size ASC)";

 /*$sql = "SELECT  cps_micr_code,SUM(cps_no_of_books) as cps_no_of_books,cps_tr_code,cps_book_size 
FROM ((select cps_micr_code,SUM(cps_no_of_books) as cps_no_of_books,cps_tr_code,cps_book_size from tb_print_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." GROUP BY ABS(cps_micr_code),cps_tr_code,cps_book_size ORDER BY cps_micr_code ASC,cps_tr_code ASC,cps_book_size ASC) UNION ALL (select cps_micr_code,SUM(cps_no_of_books) as cps_no_of_books,cps_tr_code,cps_book_size from tb_payorder_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." GROUP BY ABS(cps_micr_code),cps_tr_code,cps_book_size ORDER BY cps_micr_code ASC,cps_tr_code ASC,cps_book_size ASC)) report GROUP BY ABS(cps_micr_code),cps_tr_code,cps_book_size ORDER BY ABS(cps_micr_code) ASC,ABS(cps_tr_code) ASC,ABS(cps_book_size) ASC";*/
 $sql = "select cps_micr_code,SUM(cps_no_of_books) as cps_no_of_books,cps_tr_code,cps_book_size from tb_print_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." GROUP BY ABS(cps_micr_code),cps_tr_code,cps_book_size ORDER BY ABS(cps_micr_code) ASC,ABS(cps_tr_code) ASC,ABS(cps_book_size) ASC";
    ...
    /*  print_r($result);
      exit;*/
               ...
               // echo "SELECT branch_name from tb_branchdetails where ABS(branch_micr)='".(int)$result[0]->cps_micr_code."'";

                $branchDetails = $db->get_row("SELECT branch_name from tb_branchdetails where ABS(branch_micr)='".(int)$result[0]->cps_micr_code."'");
                ...
                $data .= '<tr><td class="thwidthtd" colspan="12" style="font-size:35px;"><b>'.$branch_name.' - '.$result[0]->cps_micr_code.'</b></td></tr>' ;
   ...
   // $sqlbranch = "(select distinct ABS(cps_micr_code) from tb_print_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." ORDER BY cps_micr_code ASC)";
     $sqlbranch = "select distinct ABS(cps_micr_code) as cps_micr_code from tb_print_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." ORDER BY ABS(cps_micr_code) ASC";
...
if($resultbranch = $db->get_results($sqlbranch)){
  ...
  foreach ($resultbranch as $rowbranch) {
            ...
            $searchStringIndv = $searchString." and ABS(cps_micr_code) = '".(int)$rowbranch->cps_micr_code."'";
           /* $sql = "(select cps_micr_code,sum(cps_no_of_books) as cps_no_of_books,cps_tr_code,cps_book_size from tb_print_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchStringIndv." GROUP BY cps_micr_code,cps_tr_code,cps_book_size ORDER BY cps_micr_code ASC,cps_tr_code,cps_tr_code ASC,cps_book_size ASC)";*/

          /*   $sql = "SELECT  * 
FROM ((select cps_micr_code,cps_no_of_books,cps_tr_code,cps_book_size from tb_print_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." GROUP BY cps_micr_code,cps_tr_code,cps_book_size ORDER BY cps_micr_code ASC,cps_tr_code ASC,cps_book_size ASC) UNION ALL (select cps_micr_code,cps_no_of_books,cps_tr_code,cps_book_size from tb_payorder_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchString." GROUP BY cps_micr_code,cps_tr_code,cps_book_size ORDER BY cps_micr_code ASC,cps_tr_code ASC,cps_book_size ASC)) report ORDER BY ABS(cps_micr_code) ASC,ABS(cps_tr_code) ASC,ABS(cps_book_size) ASC";*/

  $sql = "select ABS(cps_micr_code) as cps_micr_code,SUM(cps_no_of_books) as cps_no_of_books,cps_tr_code,cps_book_size from tb_print_req_collection where cps_date between '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'".$searchStringIndv." GROUP BY ABS(cps_micr_code),cps_tr_code,cps_book_size ORDER BY ABS(cps_micr_code) ASC,ABS(cps_tr_code) ASC,ABS(cps_book_size) ASC";
...
print_r($sql);
           ...
           //   print_r($result);
             // exit;
                        ...
                        $data .= '<tr><td class="thwidthtd" colspan="11" style="font-size:35px;"><b>'.$branch_name.' - '.$result[0]->cps_micr_code.'</b></td></tr>' ;
