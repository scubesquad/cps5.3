	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		if(!$entries_found)
		{
			foreach ($data as $key => $explode) {
				...
				cps_issue_date,cps_date, cps_micr_account_no, cps_process_user_id, cps_pr_code, cps_product_code, 
				...
				'$dates','$dates', '$explode[5]', '$cpsprocess_user_id', '$explode[34]', '$explode[36]', 
...
function DivPattern($pattern = 1,$id,$objprintdata){
	...
	function getBranches()
	{
		...
		$rowgetbranch =  $db->get_results("SELECT distinct(b.branch_sub_code), b.branch_id, b.branch_name FROM tb_branchdetails b INNER JOIN tb_uploadingdata prc ON b.branch_code = prc.cps_branchmicr_code");
		foreach($rowgetbranch as $eachbranch){
			echo '<option class="brn" value="' . $eachbranch->branch_sub_code . '">' . $eachbranch->branch_name . '</option>';
		} 
	}
