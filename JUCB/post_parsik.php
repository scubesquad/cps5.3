	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		if(!$entries_found)
		{
			foreach ($data as $key => $explode) {
				...
				$branchMICRCode = $db->get_row("SELECT branch_micr FROM tb_branchdetails WHERE branch_sub_code = '".$branch_sub_code."'");
				...
				'$explode[0]','$branchMICRCode->branch_micr','$cpsbranch_code','$explode[7]', '$explode[6]',
				'$name',
