...
{
	$sqlquery ="UPDATE tb_payorder_reprintque SET cps_reprint_approved = 1 WHERE id IN (".$_POST["ids"].") ";
	$db->query($sqlquery);
...
{
	$sqlquery ="delete from tb_payorder_reprintque WHERE id IN (".$_POST["ids"].") ";
	$db->query($sqlquery);
	echo '{"status":"true"}';
	$db->closeDb();
	exit();
