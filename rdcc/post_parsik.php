...
error_reporting(E_ALL ^ E_NOTICE);
	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
			...
			if($result = $db->get_results("select * from tb_print_req_collection where cps_account_no = '$explode[22]' and cps_chq_no_from = '$explode[26]' "))
			{
			
			//print_r($result);
				echo "<p>CPS Name : ".$result[0]->cps_act_name."</p>";
				echo "<p>ACC No   : ".$result[0]->cps_micr_account_no."</p>";
				echo "<p>Cheque From : ".$result[0]->cps_chq_no_from."</p>";
				echo "<p>Cheque To : ".$result[0]->cps_chq_no_to."</p>";
				echo "<p>CPS Date : ".$result[0]->cps_date."</p>";
		...
		if(!$entries_found)
		{
			foreach ($data as $key => $explode) {
				$branch_master_id = substr($explode[22], 0,3);
				$result_branch = $db->get_row("select branch_code, branch_atparmicrcode from tb_branchdetails where branch_master_id = '$branch_master_id' ");
				...
				$cps_micr_code = $result_branch->branch_atparmicrcode;
			    $cps_branchmicr_code = $result_branch->branch_code;
			    $dates = date('Y-m-d');
				...
				$explode[2] = str_replace("'", "\'", $explode[2]);
				$explode[3] = str_replace("'", "\'", $explode[3]);
				...
				cps_act_telephone_res, cps_act_telephone_off, cps_act_mobile, cps_pr_code

				)values(
				'$explode[0]','$cps_micr_code','$cps_branchmicr_code','$explode[23]', '$explode[31]','$explode[22]',
				 '$name', 
				'$explode[2]','$explode[3]','$addresses1','$addresses2','$addresses3',
				'$addresses4','$addresses5','$explode[12]','$explode[13]','$explode[24]', '$explode[25]',
				'$explode[29]','$explode[26]','$explode[27]',
				'$dates','$dates', '$cps_micr_account_no', '$cpsprocess_user_id', '$explode[4]',
				'$explode[5]', '$explode[6]', 
				'$explode[14]', '$explode[15]', '$explode[16]', '$explode[30]'
				)";				  
 				$db->query($sql) ;
			}
		}
