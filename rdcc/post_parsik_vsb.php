...
error_reporting(E_ALL ^ E_NOTICE);
	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
			...
			$add1 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[7])); 
			$add2 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[8])); 
			$add3 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[9]));
			$add4 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[10]));
			$add5 = str_replace("-"," ",preg_replace('/[^A-Za-z0-9\-\']/',  ' ', $explode[11]));
		...
		{
			$dates = date('Y-m-d');
			$cps_branchmicr_code = $_POST['ddlBranchName'];
			$getbankcode =  $db->get_results("SELECT bank_code FROM tb_bankdetails");
			$bank_code = $getbankcode->bank_code;
			$getbranchcitycode = $db->get_results("SELECT branch_City_Code FROM tb_branchdetails");
			$branch_City_Code = $getbranchcitycode->branch_City_Code;
			$cps_micr_code = $branch_City_Code.$bank_code.$cps_branchmicr_code;
			foreach ($data as $key => $explode) 
			{
				$cps_micr_account_no = substr($explode[22], 9,6);
			    $sql="insert into tb_uploadingdata(cps_unique_req, cps_micr_code, cps_branchmicr_code, 
			    	  cps_account_no, cps_act_name, cps_no_of_books, cps_dly_bearer_order, cps_book_size, 
			    	  cps_tr_code, cps_atpar, cps_act_jointname1, cps_act_jointname2, cps_auth_sign1, 
			    	  cps_auth_sign2, cps_auth_sign3, cps_act_address1, cps_act_address2, cps_act_address3,
					  cps_act_address4, cps_act_address5, cps_act_city, cps_act_pin, cps_act_telephone_res, 
					  cps_act_telephone_off, cps_act_mobile, cps_chq_no_from, cps_chq_no_to, 
					  cps_micr_account_no, cps_date, cps_process_user_id, cps_issue_date)				      

					  values('$explode[0]', '$cps_micr_code', '$cps_branchmicr_code', $explode[22]', '$explode[1]', '$explode[24]', '$explode[29]', 
					  '$explode[25]', '$explode[23]', '$explode[31]', '$explode[2]', '$explode[3]', '$explode[4]',
					  '$explode[5]', '$explode[6]', '$add1', '$add2', '$add3', '$add4', '$add5', '$explode[12]',
					  '$explode[13]', '$explode[14]', '$explode[15]', '$explode[16]', '$explode[26]', 
					  '$explode[27]', '$cps_micr_account_no', '$dates', '$cpsprocess_user_id', '$dates')";
